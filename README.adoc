...
It consists of three microservices, `system`, `inventory`, and `query`. The `system` microservice
reports its given properties and checks to make sure the application is ready. The `system` microservice also registers
itself on startup with the `inventory` microservice that keeps track of all instances of the `system` microservice. Finally,
the `query` microservice is a https://microservices.io/patterns/apigateway.html#variation-backends-for-frontends[backend for frontend^] service.
It communicates with the backend `inventory` microservice on the caller's behalf.

image::QueryService.png[Reactive Query Service,align="center"]
...
The `query` service communicates with the `inventory` whenever a request 
is made to retrieve the systems.

The implementations of the application and its services are provided for you in the `start` directory.
The application also uses the Apache Kafka and ZooKeeper services to distribute the inventory results and system status.
...
The ReactiveX API and the required Jersey libraries are included as dependencies to your [hotspot file=0]`query/pom.xml` file. Look for the dependencies with `artifactIDs` of [hotspot=rxjava file=0]`rxjava`, [hotspot=77-81 file=0]`jersey-client`, [hotspot=82-86 file=0]`jersey-rx-client-rxjava` and [hotspot=87-91 file=0]`jersey-rx-client-rxjava2`.
...
The `query`, `system`, and `inventory` microservices will be built in a Docker container. If you want to learn more about Docker containers, you can check out the https://openliberty.io/guides/containerize.html[Containerizing microservices^] guide.

Start your Docker environment.

To build the application, run the following script from the command line in the `start` directory.
...
Run the following script to build and containerize the application:
...
You can use `curl -X GET \http://localhost:9080/query/systemLoad` command if available on the system, to get the system loads. The https://www.getpostman.com[Postman^] application can also be used. The request will take some time for the job results to return.
...
* [hotspot=testCreateJob file=0]`testLoads()` verifies that the query service returns the correct systems to represent the `highest` and `lowest` attributes.
...
Tests run: 1, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 3.88 s - in it.io.openliberty.guides.gateway.GatewayServiceIT
