...
Reactive programming is a subset of asynchronous programming and a paradigm where data availability controls execution. In other words, execution advances when new information becomes available. In practice, this is achieved by leveraging data streams and propagating change. Reactive programming is a convenient way to implement an asynchronous and non-blocking system. This means that it can better utilize the resources of the hardware.
In more traditional systems, a consumer would have to request for data from a producer. In a reactive model, the consumer would subscribe to the producer, and push data to its subscribers.
The subscription model simplifies coordination between different services and is another great advantage of pursuing reactive programming. Instead of having to worry about implementation and configuring processes to communicate with one another, you as the programmer will only have to worry about implementing business logic.
Although reactive programming is very useful, it is worth noting that it cannot be applied blindly to every scenario. If you were to build a distributed system using only reactive programming, your system would become vulnerable to error and can be difficult to maintain. This is the drawback of the simplicity of implementation, you also lose transparency and resilience.
