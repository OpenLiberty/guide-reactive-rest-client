...
When the service is ready, you will see an output similar to the one below:
...
You can also use `curl -X GET \http://localhost:9080/query/systemLoad` command if available on the system to get the system loads. The https://www.getpostman.com[Postman^] application can also be used. The request will take some time for the job results to return.
