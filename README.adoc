...
Reactive programming takes the idea of asynchronous communication and builds on top of it. In its simplest form, reactive programming is utilizing asynchronous data streams. Inputs and outputs or transient data between different processes are processed as data streams. +
A stream is a series of events ordered in time. It has three possible types of entries: an error, a flag for the end of the stream, or a datatype value. The value is the type of object that comprises the stream, an example of which could be a mouse-click or a form submission. These values, coupled with the fact that streams can be empty, allow for a lot of flexibility in your program as you can more easily deal with problems that are usually troublesome. Handling inconsistent load is simplified because streams can handle variable length. Errors are easy to handle because streams can output errors that are easy to catch. Also, most reactive libraries provide a wealth of tools to let you handle errors more efficiently.
Another benefit of reactive programming is that the usage of these streams lets you ignore a lot of implementation details that are otherwise tedious to handle. Instead of having to define the way that each sub-system has to communicate with one another, you need only to configure the streams that each system will consume from and output to.
