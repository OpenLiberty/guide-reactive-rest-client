...
The [hotspot=systemLoadMethod file=1]`systemLoad` endpoint asynchronously processes the data that is retrieved by the [hotspot=getsystem file=1]`InventoryClient` interface and serves that data after all of the services respond. 
The [hotspot=thenAcceptAsync file=1]`thenAcceptAsync()` and [hotspot=exceptionally file=1]`exceptionally()` methods together behave like an asynchronous try-catch block. 
...
After each thread is completed, the [hotspot=countdown1 hotspot=countdown2 file=1]`countdown()` method counts the [hotspot=countdownlatch file=1]`CountDownLatch` object down towards `0`. 
