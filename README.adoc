...
The `CountDownLatch` object counts down towards `0` after each completion of a thread via the [hotspot=countdown1 hotspot=countdown2 file=1]`countdown` method call. 
...
Thus, any required processing of data is performed inside the `subscribe()` method. 
In this case, it is simply saving the data in the temporary [hotspot=holderClass file=0]`Holder` class. 
