...
//Mention more third party stuff
You will learn how to use reactive JAX-RS client to access remote RESTful services using asynchronous method calls. You'll update an existing usage of the client to call the `rx()` method which will wrap your response in a `CompletionStage`. A `CompletionStage` is an interface that will allow you to use the result of your remote service call at some point in the future.
