...
You will also need to pass an `RxObservableInvoker` into the `rx()` call. This is to update the invoker so it knows to expect an `Observable` instead of a `CompletionStage`. The `RxObservableInvoker` object is provided by Jersey, the third-party library you will be using. Other libraries provide equivalent invokers like http://cxf.apache.org/docs/jax-rs.html[Apache CXF^], which has a `ObservableRxInvoker`.
