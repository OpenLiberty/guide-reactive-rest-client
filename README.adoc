...
pluggable reactive JAX-RS client APIs provider that's published by https://eclipse-ee4j.github.io/jersey[Eclipse Jersey^].
...
Create an [hotspot file=0]`InventoryClient` class, which retrieves inventory data,
...
See the http://localhost:9085/health[^] and http://localhost:9080/health[^] URLs to confirm that
the `inventory` and `query` microservices are up and running.
Once the microservices are up and running, you can access the application by making requests to the `query/systemLoad` endpoint at the
...
// Static guide instruction
ifndef::cloud-hosted[]
See the http://localhost:9080/health[^] URL to confirm that the `query` microservice is up and running.
Once the `query` microservice is up and running, you can access the application by making requests to the `query/systemLoad` endpoint at the
http://localhost:9080/query/systemLoad[http://localhost:9080/query/systemLoad^] URL. 
endif::[]
