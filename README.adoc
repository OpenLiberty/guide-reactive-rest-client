...
The ReactiveX API and the required Jersey libraries are included as dependencies to your [hotspot file=0]`query/pom.xml` file. Look for the dependencies with `artifactIDs` of [hotspot=69-72 file=0]`rxjava`, [hotspot=73-77 file=0]`jersey-client`, [hotspot=78-82 file=0]`jersey-rx-client-rxjava` and [hotspot=83-87 file=0]`jersey-rx-client-rxjava2`.
...
* [hotspot=testLoads file=0]`testLoads()` verifies that the query service returns the correct systems to represent the `highest` and `lowest` attributes.
