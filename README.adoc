...
The ReactiveX API and the required Jersey libraries are included as dependencies to your [hotspot=defaultHttpPort hotspot=defaultHttpsPort hotspot=appContextRoot file=1]`pom.xml` file. Look for the depedencies with `rxjava`, `jersey-client`, `jersey-rx-client-rxjava` and `jersey-rx-client-rxjava2`respectively as their artidactIDs.
