...
:page-seo-title: Consuming RESTful Java microservices asynchronously using reactive JAX-RS client
...
== Updating the web client to use third-party reactive libraries

JAX-RS supports the usage of third-party reactive libraries like https://github.com/ReactiveX/RxJava[RxJava^]. This is accomplished by reactive providers. Most JAX-RS implementors publish reactive providers for RxJava, including Apache CXF, Eclipse Jersey, and RESTEasy. Liberty has tested RxJava providers from CXF and Jersey. This example shows how to use Jersey's RxJava provider.
...
Update the client to accommodate the custom object types that you are trying to return. You'll also need to register the type of object that you want inside the client invocation.
...
Instead of using `thenAcceptAsync()`, `Observable` uses the [hotspot=subscribe file=0]`subscribe()` method to asynchronously process data. Thus, any required processing of data will be performed in the [hotspot=subscribe file=0]`subscribe()` method. In this case, it is simply saving the data in the temporary [hotspot=holder hotspot=holderClass file=0]`holder`. The temporary [hotspot=holder hotspot=holderClass file=0]`holder` is used to store the value returned from the client because values cannot be returned inside [hotspot=subscribe file=0]`subscribe()`. The highest and lowest load systems are updated in the [hotspot=updateHighestCall hotspot=updateHighestMethod file=0]`updateHighest()` and [hotspot=updateLowestCall hotspot=updateLowestMethod file=0]`updateLowest()` methods respectively.
