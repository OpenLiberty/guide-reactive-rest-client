...
In the client, notice how the [hotspot=getJobsCompletionStage file=0]`getJobs()`, [hotspot=getJobCompletionStage file=0]`getJob()` and [hotspot=createJobCompletionStage file=0]`createJob()` return a `CompletionStage<T>`. This interface represents a unit or stage of a computation. Once the associated computation completes, the value contained can then be retrieved. Alternatively, the CompletionStage can be chained with additional stages using the [hotspot=thenApplyAsync file=1]`thenApplyAsync()` method for further processing. Exceptions can be handled in a callback provided to the [hotspot=exceptionally file=1]`exceptionally()` method. The `thenApplyAsync()` and `exceptionally()` methods together behave like an asynchronous try-catch block. It is important to note that when you return a CompletionStage in the resource, it doesn't guarantee that computation is complete and that the response has been built. Rather, it just means that the service will respond to the caller once the CompletionStage has completed.
...
The ReactiveX API and the required Jersey libraries are included as dependencies to your [hotspot file=2]`pom.xml` file. Look for the dependencies with [hotspot=111 file=2]`rxjava`, [hotspot=116 file=2]`jersey-client`, [hotspot=121 file=2]`jersey-rx-client-rxjava` and [hotspot=126 file=2]`jersey-rx-client-rxjava2` respectively as their artidactIDs.
