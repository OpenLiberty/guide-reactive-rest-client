...
In the [hotspot file=1]`GatewayJobResource` class, the desired return type is returned in the [hotspot=thenApplyAsync file=1]`thenApplyAsync()` method.
...
Update the client to accommodate the custom object types you are trying to return. Also register the type of object you want inside the client invokation.

[role="code_command hotspot hotspot=1", subs="quotes"]
----
