...
A stream is a series of events ordered in time. It has three possible types of entries: an error, a flag for the end of the stream, or a value of a datatype. The value is the type of object that comprises the stream, an example of which could be a mouse-click or a form submission. These values, coupled with the fact that streams can be empty, allow for a lot of flexibility in your program as you can more easily deal with problems that would usually be annoying to deal with. Inconsistent load is simplified because streams can handle variable length. Errors are easy to handle because streams can emit errors that are easy to catch.
