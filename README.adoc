...
:page-description: Learn how to use reactive JAX-RS client to invoke RESTful microservices over HTTP asynchronously.
...
== Changing the client to use reactive JAX-RS client
...
Instead of using a `CompletionStage` return type, JAX-RS API allows you to use a type from a third-party library such as https://github.com/ReactiveX/RxJava[RxJava^]. You can achieve this by passing the custom type into your call to `rx()`.
