...
You will learn how the default reactive JAX-RS client accesses remote RESTful services using asynchronous method calls. The default client uses `CompletionStage` and a basic invoker method `rx()` to perform its tasks. You will then update this client to use third party libraries to become more customizable. Instead of using `CompletionStage`, which are single use, you will instead use `Observable` which supports streams and is thus more powerful. You will also update the default invoker with the associated `RxObservableInvoker`.
...
In the [hotspot file=0]`JobClient` class, there are additional methods used to create, and configure the JAX-RS client. Also note how the other methods all contain an [hotspot=rxGetJobsCompletionStage hotspot=rxGetJobCompletionStage hotspot=rxCreateJobsCompletionStage file = 0]`rx()` call. This is the default reactive invoker for CompletionStage and this method is what retrieves the `CompletionStageRxInvoker` and allows for these methods to fucntion correctly with the CompletionStage return type.

In the [hotspot file=1]`GatewayJobResource` class, the desired return type is returned in the `thenApplyAsync()` method.
...
By using Jersey, the client can be updated to support https://github.com/ReactiveX/RxJava[RxJava^] objects instead of only CompletionStage types.


The ReactiveX API and the required Jersey libraries are included as dependencies to your [hotspot file=2]`pom.xml` file. Look for the depedencies with [hotspot=111 file=2]`rxjava`, [hotspot=116 file=2]`jersey-client`, [hotspot=121 file=2]`jersey-rx-client-rxjava` and [hotspot=126 file=2]`jersey-rx-client-rxjava2` respectively as their artidactIDs.
...
Update the client to accommodate the custom object types you are trying to return. Also register the type of object you want inside the `webTarget`.
...
The changes involve changing the return types of [hotspot=getJobs file=3]`getJobs()`, [hotspot=getJob file=3]`getJob()` and [hotspot=createJob file=3]`createJob()` from `CompletionStage<T>` to `Observable<T>`. The `http://reactivex.io/RxJava/javadoc/io/reactivex/Observable.html[Observable^]` type is an object that is part of ReactiveX and, as will be seen later, is a more flexible data type than the CompletionStage.
The [hotspot=rxGetJobs hotspot=rxGetJob hotspot=createJob file=3]`rx()` invoker also must contain `RxObservableInvoker.class` as an argument; This is to invoke the specific invoker for the Observable class provided by Jersey. The invoker depends on the type of object you wish to return, if you wanted to return a `http://reactivex.io/RxJava/2.x/javadoc/io/reactivex/Flowable.html[Flowable^]` instead, you would need to pass `RxFlowableInvoker.class`. For this to work however, the invoker would need to have a provider that is registered with the client. Notice in the `webTarget()` method the `register(RxObservableInvokerProvider)` call that allows for the client to recognize the `RxObservableInvoker` class. Without registering the invoker provider, the invoker will not be recognized.


== Updating the JAX resources

Now that the client methods return `Observable<T>`, you must update the resource to accommodate these changes. Instead of using `thenApplyAsync()`, observables use the `subscribe()` method to asynchronously process data. Thus any required processing of data will be performed in the `subscribe()` call; In this case, it is simply saving the data in the temporary holder.
