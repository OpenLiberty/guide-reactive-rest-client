...
The [hotspot=countdownlatch file=1]`CountDownLatch` object will countdown towards 0 after the completion of a thread via the [hotspot=countdown1 hotspot=countdown2 file=1]`countdown` method call. 
...
In this case, no such task is present, but a simple task can be given to occupy the thread.
...
The return types of the [hotspot=getSystem file=0]`getSystem()` method are now an `Observable` object instead of a `CompletionStage` interface. 
...
The [hotspot=rx file=0]`rx()` method now needs to contain `RxObservableInvoker.class` as an argument. This is to call the specific invoker, `RxObservableInvoker`, for the Observable class provided by Jersey. 
In the [hotspot=getSystem file=0]`getSystem()` method, the [hotspot=register file=0]`register(RxObservableInvokerProvider)` call registers the `RxObservableInvoker` class, which means that the client can recognize the invoker provider.
...
The temporary `Holder` class is used to store the value returned from the client because values cannot be returned inside the `subscribe()` method. 
