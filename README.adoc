...
You'll learn how to create a reactive JAX-RS client application using the default JAX-RS reactive provider and a pluggable reactive provider for RxJava that's published by https://eclipse-ee4j.github.io/jersey[Eclipse Jersey^]. 
...
Create an [hotspot file=0]`InventoryClient` class, which is used to retrieve inventory data, and a [hotspot file=1]`QueryResource` class, which queries data from the `inventory` service.
...
The [hotspot=systemLoadMethod file=1]`systemLoad` endpoint asynchronously processes the data that is retrieved by the [hotspot=getsystem file=1]`InventoryClient` interface and serves that data after all of services respond. 
...
The data is processed in the [hotspot=thenAcceptAsync file=1]`thenAcceptAsync()` method only after `CompletionStage` interface finishes retrieving it. 
...
If you want to learn more about Docker containers, check out the https://openliberty.io/guides/containerize.html[Containerizing microservices^] guide.
...
The JSON output for each of these attributes features the respective `hostname` and `systemLoad` of the system.

When you are done checking out the application, run the following command to stop the `query` microservice. 
We will leave the `system` and `inventory` services running because they will be used when the application is rebuilt later in the guide:
